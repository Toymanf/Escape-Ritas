  ...
  if (!strcmp(topic, mqttFullTopic("Simon/")) & message == "start") {
    ...
    while (!mqttDecodeSolve) {
      ...
      if (ra) {
        ...
        if (baskRelic) {
        for (int i = 53; i < 70; i++) {
          lightColumn(1, i, 0, 255, 200);
          lightColumn(2, i, 0, 255, 200);
          lightColumn(3, i, 0, 255, 200);
          lightColumn(4, i, 0, 255, 200);
        }
        //FastLED.show();
        }
        if (anubisRelic) {
        for (int i = 36; i < 53; i++) {
          lightColumn(1, i, 96, 255, 200);
          lightColumn(2, i, 96, 255, 200);
          lightColumn(3, i, 96, 255, 200);
          lightColumn(4, i, 96, 255, 200);
        }
        //FastLED.show();
        }
        if (horusRelic) {
        for (int i = 19; i < 36; i++) {
          lightColumn(1, i, 160, 255, 200);
          lightColumn(2, i, 160, 255, 200);
          lightColumn(3, i, 160, 255, 200);
          lightColumn(4, i, 160, 255, 200);
        }
        //FastLED.show();
        }
        if (raRelic) {
...
void lightDiagonal(int diagonal, int V = 255) { //This is only for Simon. v= brightness
  int H, firstTile;
  switch (diagonal) {
    case 1:
      firstTile = 5;
...
void blinkDiagonal(int diagonal) {
  ...
  FastLED.show();
  //wait(500);//Moved this to after checking tiles for held down
}
...
void decodeMode() {
  ...
  for (int j = 0; j < 6; j++) {
    ...
    while (wloop) {
      ...
      temp = readTile(tilePressure);  //Threshold is set here Was 3200
  ...
  for (int currentRound = 1; currentRound <= rounds; currentRound++) {//Intializes Simon and plays until solved or mistake or timeout
    ...
    for (int j = 0; j < currentRound; j++) {  //Wait for tile input
      ...
      while (currentTile == 0) {  //wait for player input. Code will loop here until a diagonal tile is pressed
        ArduinoOTA.handle();      //This is where I would add a timeout for user input which would force a reset of array and restart of Simon
        client.loop();
        checkConnection();        
...
void StepPuzzle(int players) {  //The first puzzle of the room, this function contains a parameter corresponding to the number of
  ...
  int activeTiles[8] = { 26, 18, 24, 30, 20, 28, 2, 6 }; 
...
void evaluate(int players) {
  ...
  int activeTiles[8] = { 27, 18, 14, 22, 16, 24, 11, 20 };  //Was  int activeTiles[8] = { 20, 18, 14, 22, 16, 24, 11, 27 }; 
  for (int j = 0; j <= players-1; j++) {
    int i=activeTiles[j];
    validInputs[i-1]=1;
    }
