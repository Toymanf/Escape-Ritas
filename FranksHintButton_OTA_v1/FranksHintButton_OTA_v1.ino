...
*/
  ...
  */
  ...
  ArduinoOTA.setHostname("LibraryHintButton_OTA");
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), MQTT_USER, MQTT_PASSWORD)) {
      ...
      client.publish("/Egypt/Library/", "Library Hint Button Online");
      // ... and resubscribe
      client.subscribe("/Egypt/Library/Hint/");
    } else {
...
void callback(char* topic, byte *payload, unsigned int length) {
  ...
  if (message == "hintSuggested") {
    checkConnection();
    client.publish("/Egypt/Library/","Mqtt hint suggested");
    mqttHint=true;
  }
  else if (message == "on"){
...
void readButtons(){
  if (digitalRead (D5)==LOW && !hintR){
    hintR=true;
    client.publish("/Egypt/Library/","Library hint button pressed");
    mqttHint=false;
    flashHint=true;
    flash();
    }
    if (digitalRead (D7)==LOW && !hintR){
    hintR=true;
    client.publish("/Egypt/Library/","Archive hint button pressed");
    mqttHint=false;
    flashHint=true;
    flash();
    }
    ...
    estop=true;
    client.publish("/Egypt/Library/","Estop button pressed");
    }
  }
