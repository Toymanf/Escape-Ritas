...
if ((!client.connected() ) && (WiFi.status() == WL_CONNECTED) ){
  ...
  if ((currentMillis - previousMillis >=interval)) {
    client.publish("/Egypt/Tomb/", "Tomb Sliding Puzzle Watchdog");
    previousMillis = currentMillis;
...
void loop() {
  ...
  client.publish("/Egypt/Tomb/", "Sliding puzzle lid opened");
  while (!complete){
...
while (solved){
  ...
  if (!digitalRead(lid)){//if lid is closed
 ...
 client.publish("/Egypt/Tomb/", "Sliding puzzle lid still open");   
